/**
 * Talks to the management API running inside the one persistent
 * shared SSH server (docker/ssh-shared), over Railway's private
...
class SshManagerClient {
  ...
  async createUser(username, password) {
    ...
    if (!res.ok) {
      throw new Error(`SSH user creation failed: ${data.error || res.status}`);
    }
    return data;
  }
  ...
  async deleteUser(username) {
    ...
    const data = await res.json();
    if (!res.ok) {
      throw new Error(`SSH user deletion failed: ${data.error || res.status}`);
    }
    return data;
  }
}
